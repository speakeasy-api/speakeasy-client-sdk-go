...
```go
speakeasyclientsdkgo.WithServerURL("https://api.example.com")
```
...
```go
speakeasyclientsdkgo.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
speakeasyclientsdkgo.WithServer("my-server")
```
...
```go
speakeasyclientsdkgo.WithClient(httpClient)
```
...
```go
speakeasyclientsdkgo.WithSecuritySource(/* ... */)
```
...
```go
speakeasyclientsdkgo.WithWorkspaceID(/* ... */)
```
...
```go
speakeasyclientsdkgo.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
speakeasyclientsdkgo.WithTimeout(30 * time.Second)
```
